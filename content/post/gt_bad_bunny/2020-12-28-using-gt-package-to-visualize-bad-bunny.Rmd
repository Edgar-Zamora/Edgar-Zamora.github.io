...
To change the name of each column, you use the `col_label()` function. It is 'important to note that while columns can be freely relabeled, we continue to refer to the columns by their original column names'^[More information can be found at: https://gt.rstudio.com/reference/cols_label.html]. With that in mind, everything to the left of the equal sign should refer to the selected column by name while the right side is the new label of the aforementioned column like so `current name = new name`. While also relabeling columns, you can allow the option to accept `md()` and `html()` helper functions, so that you can add cool things like emojis^[You can search for the hex or decimal code on Google!!
